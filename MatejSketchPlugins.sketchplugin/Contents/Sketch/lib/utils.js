...
// interacting with NSUserDefaults
utils.localStorage = {
  get : function(key){
    var defaults = [NSUserDefaults standardUserDefaults],
    defaultValue = [defaults objectForKey: [plugin identifier] + '.' + key];
    if (defaultValue) {
      // need to return mutable dictionary
      var mutableDefaultValue = [[NSMutableDictionary alloc] init];
      for(var key in defaultValue) {
        mutableDefaultValue[key] = defaultValue[key];
      }
      defaultValue = mutableDefaultValue;
    }
    return defaultValue;
  },
  set : function(key, value){
    var defaults = [NSUserDefaults standardUserDefaults];
    var response = [defaults setObject: value forKey: [plugin identifier] + '.' + key];
...
utils.common = {
  ...
  containsString : function(str, substring){
    var strRange = [str rangeOfString:substring];
    if (strRange.length > 0) { // hack to simulate NSNotFound
      return true;
    }
    return false;
  },
